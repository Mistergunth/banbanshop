// lib/screens/buyer/edit_buyer_profile_screen.dart
...
class _EditBuyerProfileScreenState extends State<EditBuyerProfileScreen> {
  ...
  void initState() {
    super.initState();
    _fullNameController = TextEditingController(text: widget.buyerProfile.fullName);
    _emailController = TextEditingController(text: widget.buyerProfile.email);
    _phoneController = TextEditingController(text: widget.buyerProfile.phoneNumber?.replaceAll('+66', ''));
  }
  ...
  Future<void> _updateProfile() async {
    ...
    try {
      ...
      if (newFullName != widget.buyerProfile.fullName) {
        await user.updateDisplayName(newFullName);
        await FirebaseFirestore.instance.collection('buyers').doc(user.uid).update({'fullName': newFullName});
        hasChanges = true;
      }

      if (newEmail != widget.buyerProfile.email) {
  ...
  Future<void> _verifyAndupdatePhone() async {
    ...
    if (newPhoneWithCountryCode == widget.buyerProfile.phoneNumber) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('แก้ไขโปรไฟล์ผู้ซื้อ'),
        centerTitle: true,
      ),
